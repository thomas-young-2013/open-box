...
class Dispatcher(object):
	...
	def __init__(self, new_result_callback, run_id='0', ping_interval=10, nameserver='localhost', nameserver_port=None,
		...
		if logger is None:
			self.logger = logging.getLogger('[OpenBox] dispatcher')
		else:
			self.logger = logger
		...
		self.pyro_id = "openbox.run_%s.dispatcher" % self.run_id
	...
	def discover_workers(self):
		...
		while True:
			...
			with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
				worker_names = ns.list(prefix="open-box.run_%s.worker." % self.run_id)
