...
class SobolSampler(Sampler):
    ...
    def __init__(self, config_space: ConfigurationSpace,
                 size, lower_bounds=None, upper_bounds=None, seed=None):
        """
        Parameters
        ----------
        config_space : ConfigurationSpace
        ...
        """
        super().__init__(config_space, size, lower_bounds, upper_bounds)
        if seed is None:
            seed = np.random.randint(int(1e6))
        self.seed = seed

    def _generate(self):
        sobol = Sobol(min_skip=self.seed, max_skip=self.seed)
...
class LatinHypercubeSampler(Sampler):
    ...
    def _generate(self):
        ...
        X = lhs.generate(self.search_dims, self.size)
        return X
