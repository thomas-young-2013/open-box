...
import argparse
"""
currently: run this script by `python test/optimizer/test_smbo_lightgbm_visualization.py --datasets spambase --n <loops>`
after the job done
"""
...
parser.add_argument('--n', type=int, default=50)  # 最大的迭代次数
parser.add_argument('--time', type=int, default=60)  # 每次迭代的最大时间

args = parser.parse_args()
dataset_str = args.datasets
run_count = args.n
time_per_run = args.time
...
def eval_func(params, x, y):
    ...
    ans = 1 - balanced_accuracy_score(y_test, y_pred)
    print(ans)
    return ans
...
if __name__ == '__main__':
    ...
    for dataset in dataset_list:
        ...
        print('=' * 10, 'SMBO')
        bo = SMBO(eval, cs, max_runs=run_count, time_limit_per_trial=600, logging_dir='logs')
        bo.run()
